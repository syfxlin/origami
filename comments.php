...
if ($comment_registration == '' || ($comment_registration == '1' && is_user_logged_in())) :
    if (comments_open()) :
?>
                ...
                <div class="response-footer">
                    <div class="response-input-item">
                        <div class="form-group has-icon-right">
                            <input data-rule="required(请输入昵称)|/.{1,50}/昵称太长或太短|disinput|focus" name="author" <?php if ($logged) {
                                                                                                                    echo 'style="display:none"';
                                                                                                                } ?> id="response-author" class="form-input" type="text" value="<?php echo $comment_author; ?>" placeholder="<?php echo __('昵称', 'origami'); ?> *">
                        </div>
                        <div class="form-group has-icon-right">
                            <input name="url" <?php if ($logged) {
                                                    echo 'style="display:none"';
                                                } ?> id="response-website" class="form-input" type="text" value="<?php echo $comment_author_url; ?>" placeholder="<?php echo __('网站', 'origami'); ?>">
                        </div>
                    </div>
                    <div class="response-input-item">
                        <div class="form-group has-icon-right">
                            <input data-rule="required(请输入邮箱)|/^([A-Za-z0-9_\-\.\u4e00-\u9fa5])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,8})$/您输入的邮箱有误|disinput|focus" name="email" <?php if ($logged) {
                                                                                                                                                                                    echo 'style="display:none"';
                                                                                                                                                                                } ?> id="response-email" class="form-input" type="text" value="<?php echo $comment_author_email; ?>" placeholder="<?php echo __('邮箱', 'origami'); ?> *">
                        </div>
                        <div class="form-group has-icon-right">
    ...
    <h5><?php echo __('您必须', 'origami'); ?> <a href="<?php echo wp_login_url(); ?>"><?php echo __('登录', 'origami'); ?></a> <?php echo __('才能发表评论', 'origami'); ?></h5>
<?php endif ?>
