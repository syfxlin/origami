...
if(get_option('origami_first_install') != "ok") {
    ...
    $key = file_get_contents('http://lab.ixk.me/wordpress/Origami-install-count.php?type=get-key&site-url='.$_SERVER['HTTP_HOST'], false, $header);
...
register_nav_menus(
    array('main-menu' => esc_html__('Main Menu'))
);
// 加载主题设置
...
if (!is_admin()) {
    // 加载主要css/js文件
    wp_enqueue_style('origami-style', get_stylesheet_uri(), array(), wp_get_theme()->get('Version'));
    wp_enqueue_script('comment-reply');
    wp_enqueue_script('jquery-script', get_template_directory_uri() . '/js/jquery-3.3.1.min.js', array(), wp_get_theme()->get('Version'));
    wp_enqueue_script('origami-script', get_template_directory_uri() . '/js/main.js', array(), wp_get_theme()->get('Version'));
    wp_enqueue_script('qrcode-script', get_template_directory_uri() . '/js/qrcode.min.js', array(), wp_get_theme()->get('Version'));
    ...
    if (get_option('origami_canvas_nest') == true) {
        ...
        {
            echo '<script type="text/javascript" color="0,0,0" zindex="-1" opacity="0.5" count="99" src="' . get_template_directory_uri() . '/js/canvas-nest.js"></script>';
        }
    ...
    function origami_load_owo()
    {
        if (get_option('origami_comment_owo') == true) {
            wp_enqueue_style('owo-style', get_template_directory_uri() . '/css/OwO.min.css');
            wp_enqueue_script('owo-script', get_template_directory_uri() . '/js/OwO.min.js');
        }
    }
    // 文章目录加载
    function origami_load_tocbot()
    {
        wp_enqueue_style('tocbot-style', get_template_directory_uri() . '/css/tocbot.css');
        wp_enqueue_script('tocbot-script', get_template_directory_uri() . '/js/tocbot.min.js');
    }
    // 加载代码高亮
    function origami_load_prism()
    {
        wp_enqueue_style('prism-style', get_template_directory_uri() . '/css/prism.css');
        wp_enqueue_script('prism-script', get_template_directory_uri() . '/js/prism.js');
    }
    // 加载Lazyload
    function origami_load_lazyload()
    {
        ...
        if (strcmp($config[0], 'true') == 0) {
            wp_enqueue_script('lazyload-script', get_template_directory_uri() . '/js/lazyload.min.js');
        }
    }
    ...
    {
        $origami_footer_content = file_get_contents(get_theme_file_path()."/footer.php");
        if(stripos($origami_footer_content, "www.ixk.me") == false || stripos($origami_footer_content, "origami-theme-info") == false) {
...
function origami_async_script($tag, $handle, $src)
{
    ...
    $array_exclusions = !empty($async_exclusions) ? explode(',', $async_exclusions) : array();
    if (false === is_admin()) {
        ...
        $tag = str_replace('src=', $async_method . "='" . $async_method . "' src=", $tag);
        return $tag;
    }
    return $tag;
}
...
function origami_breadcrumbs()
{
    ...
    if (!is_home() && !is_front_page() || is_paged()) {
        global $post;
        $homeLink = home_url();
        echo ' <a href="' . $homeLink . '">' . __('Home') . '</a> ' . $delimiter . ' ';
...
function origami_footer_time_fun()
{
    ...
    if ($origami_footer_time != '') {
        echo '
            <script>// <![CDATA[
            var now = new Date();
            function createtime(){ var grt= new Date("' . $origami_footer_time . '");
...
function origami_comment($comment, $args, $depth)
{
    ...
    if ($comment->comment_author_email == $adminEmail) {
        $comment_mark = '<a target="_blank" href="/关于我" title="经鉴定，这货是站长">站长</a>';
    ...
    $comment_mark = '<div class="comment-mark" style="background:' . $comment_mark_color . '">' . $comment_mark . '</div>';
    /* 评论者标签 - end */
    ?>
    ...
    <div id="comment-<?php echo $comment->comment_ID; ?>" class="comment-level-<?php echo $depth; ?> comment-list">
        ...
        <div class="comment-right">
            ...
            <div class="comment-footer">
                ...
                <div class="comment-date"><i class="fa fa-clock-o" aria-hidden="true"></i>发表于: <?php echo get_comment_time('Y-m-d H:i'); ?></div>
                </div>
...
function origami_load_comments()
{
    if (isset($_GET['action'])) {
        if ($_GET['action'] == 'comments') {
            ...
            wp_list_comments('type=comment&callback=origami_comment&reverse_top_level=true&per_page=10&page=' . $page_index, $comments);
            echo '</div>';
            echo '<div id="comment_page_index" style="display:none">' . $page_index . '</div>';
            die;
        }
    }
}
...
function origami_content_copyright($content)
{
    if (is_single() || is_feed()) {
        $content .= '<div id="content-copyright"><span style="font-weight:bold;text-shadow:0 1px 0 #ddd;font-size: 12px;">声明:</span><span style="font-size: 12px;">本文采用 <a rel="nofollow" href="http://creativecommons.org/licenses/by-nc-sa/3.0/" title="署名-非商业性使用-相同方式共享">BY-NC-SA</a> 协议进行授权，如无注明均为原创，转载请注明转自<a href="' . home_url() . '">' . get_bloginfo('name') . '</a><br>本文地址:<a rel="bookmark" title="' . get_the_title() . '" href="' . get_permalink() . '">' . get_the_title() . '</a></span></div>';
    }
    return $content;
}
    ...
    if (is_single() || is_feed()) {
        ...
        if (preg_match_all($r, $content, $matches)) {
            foreach ($matches[1] as $num => $title) {
                $content = str_replace($matches[0][$num], substr($matches[0][$num], 0, 3) . ' id="title-' . $num . '"' . substr($matches[0][$num], 3), $content);
            }
        }
        // 删除pre块
        $matches = array();
...
function origami_prism_shortcode_func($attr, $content)
{
    ...
    $output = '<pre class="' . $line . 'language-' . $lang . '"><code class=" language-' . $lang . '">' . $content . '</code></pre>';
    return $output;
}
...
function origami_notebox_shortcode_func($attr, $content)
{
    ...
    $output = '<div class="message-box ' . $color . '"><p>' . $content . '</p></div>';
    return $output;
}
...
function origami_image_shortcode_func($attr, $content)
{
    ...
    $src = $content;
    $output = '<img src="' . $src . '" alt="' . $alt . '" is-thum="' . $is_thum . '"';
...
{
    $plugin_array['origami_image_add'] = get_template_directory_uri() . '/js/shortcode.js';
    $plugin_array['prism'] = get_template_directory_uri() . '/js/shortcode.js';
    $plugin_array['notebox_yellow'] = get_template_directory_uri() . '/js/shortcode.js';
    $plugin_array['notebox_blue'] = get_template_directory_uri() . '/js/shortcode.js';
    $plugin_array['notebox_green'] = get_template_directory_uri() . '/js/shortcode.js';
    $plugin_array['notebox_red'] = get_template_directory_uri() . '/js/shortcode.js';
    return $plugin_array;
}
...
function origami_get_other_thumbnail($post)
{
    // <img.+src=[\'"]([^\'"]+)[\'"].+is-thum=[\'"]([^\'"]+)[\'"].*>
    $image_url = false;
    preg_match_all('/\[image.+is-thum="true".+\]([^\'"]+)\[\/image]/i', $post->post_content, $matches);
...
function origami_lazyload_img()
{
    ...
    function origami_lazyload_img_process_callback($matches)
    {
        ...
        if (stripos($img_attr, "src=") === false) {
            ...
            if (stripos($img_attr, "width=") !== false
                ...
                || stripos($img_attr, "height:") !== false) {
                $load_src = 'data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==';
                $img_attr = substr_replace($img_attr, $load_src, stripos($img_attr, 'src=') + 5, 0);
            } else {
                $load_src = 'data:image/gif;base64,';
                $img_attr = substr_replace($img_attr, $load_src, stripos($img_attr, 'src=') + 5, 0);
            }
            $img_attr = 'data-src="' . $src_matches[2] . '" ' . $img_attr;
        }
        if (stripos($img_attr, "class=") !== false) {
            $img_attr = substr_replace($img_attr, 'lazyload ', stripos($img_attr, 'class=') + 7, 0);
        } else {
    ...
    {
        $regex = "/<img (.+)>/i";
        $content = preg_replace_callback($regex, "origami_lazyload_img_process_callback", $content);
        return $content;
    }
}
    ...
    if (isset($_GET['action'])) {
        if ($_GET['action'] == 'real_time_search') {
            ...
            foreach ($search_posts as $post) {
                echo '<li class="col-xlarge-12"><div id="post-'.esc_attr($post->ID).'" ';post_class('post-list-item wide-post-list-item blog-list-item post-item-left'.$post->ID);
                echo 'style="padding-bottom:60px;">';
                echo '<h3 class="font-montserrat-reg"><a href="'.esc_url(get_the_permalink($post->ID)).'">'.esc_attr(get_the_title($post->ID)).'</a></h3>';
                echo '<div class="post-list-item-meta font-opensans-reg clearfix">';
                echo '<span>'.esc_attr(get_the_date(get_option('date_format'), $post->ID)).'</span>';
                echo '<span>'.esc_attr(get_the_author_meta('nickname', $post->post_author)).'</span>';
                echo '</div>';
                echo '<div class="page-content">'.wp_trim_words($post->post_content, 100).'</div>';
                echo '<div class="myblog-post-list-buttom">';
                echo '<a href="'.esc_url(get_the_permalink($post->ID)).'" class="primary-button font-montserrat-reg hov-bk myblog-post-list-button">Read more</a>';
                echo '</div></div></li>';
            }
            echo '</ul>';
            die;
...
// New Function.php
add_menu_page(__('Origami主题', 'origami'), __('Origami主题', 'origami'), 'edit_themes', 'origami_config', 'ori_menu_fun');
// end
